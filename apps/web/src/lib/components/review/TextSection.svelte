...
<div class="right">
	<button type="button" class="action" onclick={() => addSection(section.position)}
		>{$i18nMessages.t("web:TextSection.add")}</button
	>
	[
	<button type="button" class="action" onclick={() => editSection(section.code)}
		>{$i18nMessages.t("web:TextSection.edit")}</button
	>] [
	<button type="button" class="action" onclick={() => deleteSection(section.code)}
		>{$i18nMessages.t("web:TextSection.del")}</button
	>] [
	<button type="button" class="action" onclick={() => moveSection(section.position, -1)}
		>{$i18nMessages.t("web:TextSection.up")}</button
	>
	<button type="button" class="action" onclick={() => moveSection(section.position, 1)}
		>{$i18nMessages.t("web:TextSection.down")}</button
	>
	]
</div>
<div class="editor edit-{section.code}">
	<textarea class="editing">{section.data}</textarea>
	<button type="button" onclick={() => saveSection(section.code)}
		>{$i18nMessages.t("web:TextSection.save")}</button
	>
</div>
